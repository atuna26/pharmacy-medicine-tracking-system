...
function NoPrescriptionSale() {
  const dispatch = useDispatch();
  const token = useSelector((state)=>state.token);
  const prescriptions = useSelector((state)=>state.prescriptions)
  const getSaleList = async () => {
    const response = await fetch(`http://localhost:3001/satis/recetesiz-satis`, {
      method: "GET",
  ...
  useEffect(() => {
    getSaleList();
    document.title = "EYS BETA 0.1.0 - Reçetesiz Satış";
              ...
              <StyledTableRow>
                ...
                <TableCell align="right">
                  ...
                  >
                    ...
                    <Button variant="contained">İndir</Button>
                  </Stack>
                </TableCell>
              </StyledTableRow>
            ))}
...
export default NoPrescriptionSale;
